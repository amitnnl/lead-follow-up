  ...
  return (
    <tr className="hover:bg-primary-50/20 dark:hover:bg-primary-500/5 transition-colors group">
      <td className="px-2 py-1.5">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="font-semibold text-slate-800 dark:text-white text-xs truncate max-w-[200px]">{exec.name}</div>
      ...
      <td className="px-2 py-1.5 text-xs">
        ...
        <div className="text-[10px] text-slate-400">{exec.email || '—'}</div>
      </td>
      <td className="px-2 py-1.5 text-xs">
...
function ExecutiveModal({ 
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} zClassName="z-[9999]" className="bg-white dark:bg-[#0F1420] rounded-xl shadow-2xl w-full max-w-2xl max-h-[96vh] overflow-hidden flex flex-col my-auto border border-slate-200 dark:border-slate-800">
        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/90 dark:bg-slate-900/80 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-primary-50 dark:bg-primary-500/10 border border-primary-100 dark:border-primary-500/20 flex items-center justify-center text-primary-500">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Configure sales force executive profile and financer mapping</p>
            </div>
          </div>
        ...
        <form onSubmit={onSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <div className="p-3 sm:p-4 space-y-3 overflow-y-auto flex-1 min-h-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <div>
                ...
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full h-[34px] px-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 text-xs text-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Mobile</label>
                <input type="tel" maxLength={10} pattern="^\d{10}$" title="Mobile number must be exactly 10 digits" value={formData.mobile} onChange={e => setFormData({...formData, mobile: e.target.value.replace(/\D/g, '').slice(0, 10)})} className="w-full h-[34px] px-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 font-mono text-xs text-slate-800 dark:text-white" placeholder="9876543210" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Email</label>
                <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full h-[34px] px-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 text-xs text-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Linked Financer</label>
                <select value={formData.financer_id} onChange={e => setFormData({...formData, financer_id: e.target.value})} className="w-full h-[34px] px-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 text-xs text-slate-800 dark:text-white">
          ...
          <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-1.5">
              ...
              <label htmlFor="exec_is_active" className="font-medium text-xs text-slate-600 dark:text-slate-300 cursor-pointer">Active Status</label>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={onClose} className="px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer">Cancel</button>
              <button type="submit" className="px-3.5 py-1.5 text-xs font-semibold bg-primary-600 text-white hover:bg-primary-700 rounded-lg shadow-xs transition-colors cursor-pointer">
  ...
  return (
    <div className="space-y-2.5 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h1 className="text-base sm:text-lg font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
            <div className="w-7 h-7 rounded-lg bg-primary-500/15 flex items-center justify-center">
              <UserCircle className="w-4 h-4 text-primary-600 dark:text-primary-400" />
            </div>
            Bank Executives
          </h1>
          <p className="text-xs text-slate-400 dark:text-slate-500">Manage executives working with finance companies and banks.</p>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button 
            ...
            className="bg-primary-600 hover:bg-primary-700 text-white px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
          >
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-lg p-2.5 text-center">
          <div className="text-xl font-bold text-slate-500 dark:text-slate-400">{stats.inactive}</div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div className="relative max-w-sm w-full sm:w-auto flex-1">
              ...
              <input 
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 text-slate-800 dark:text-white"
              />
              {search && (
            ...
            {showFilters && (
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as any)} className="h-[32px] px-2 text-xs bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 text-slate-800 dark:text-white">
