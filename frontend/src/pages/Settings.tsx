...
export default function Settings() {
  ...
  const inputClass = "w-full h-[32px] px-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500/20 text-xs text-slate-800 dark:text-white transition-all";
  const labelClass = "block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1";

  if (loading) return <div className="flex flex-col items-center justify-center h-80 gap-2"><div className="relative w-8 h-8"><div className="absolute inset-0 rounded-full border-[2.5px] border-primary-100 dark:border-primary-500/20" /><div className="absolute inset-0 rounded-full border-[2.5px] border-t-primary-600 animate-spin" /></div><p className="text-xs text-slate-400 font-medium">Loading settings...</p></div>;

  return (
    <div className="space-y-2.5 pb-6 select-none max-w-6xl mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h1 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
            <SettingsIcon className="text-primary-500 w-4 h-4" /> Settings
          </h1>
      ...
      {successMsg && (
        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-xs font-semibold">
          <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" /> {successMsg}
        </div>
      )}

      <div className="card overflow-hidden flex flex-col md:flex-row min-h-[480px] rounded-xl">
        
        {/* Sidebar Nav */}
        <div className="w-full md:w-52 bg-slate-50/50 dark:bg-slate-900/30 border-r border-slate-100 dark:border-slate-800 p-2 sm:p-2.5 flex flex-col">
          {['Personal', 'Global Project'].map(group => {
            ...
            return (
              <div key={group} className="mb-2.5">
                <h3 className="text-[9px] font-bold tracking-widest text-slate-400 dark:text-slate-500 uppercase mb-1 ml-1">{group}</h3>
                <nav className="space-y-0.5">
                  {groupTabs.map(tab => (
                    <button
                      ...
                      className={clsx(
                        "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer border",
                        activeTab === tab.id 
                          ? "bg-white dark:bg-slate-800 text-primary-600 dark:text-primary-400 border-slate-200 dark:border-slate-700 shadow-xs" 
                          : "text-slate-600 hover:bg-slate-100/70 dark:text-slate-400 dark:hover:bg-slate-800/40 border-transparent"
                      )}
                    >
                      <tab.icon className={clsx("w-3.5 h-3.5 shrink-0", activeTab === tab.id ? "opacity-100" : "opacity-60")} />
                      {tab.label}
                      {tab.badge && activeTab === tab.id && <span className="ml-auto px-1.5 py-0.5 text-[8px] font-extrabold uppercase rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400">{tab.badge}</span>}
                    </button>
                  ))}
                </nav>
              </div>
        ...
        <div className="flex-1 p-3 sm:p-4 overflow-y-auto">
          ...
          {/* PROFILE TAB */}
          {activeTab === 'profile' && (
            <form onSubmit={handleProfileSave} className="space-y-3 max-w-xl text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <User className="w-4 h-4 text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Profile Information</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">
                ...
                <div className="space-y-2.5">
                  ...
                  <div>
                    ...
                    <input type="text" disabled value={user?.role?.replace('_', ' ').toUpperCase() || ''} className="w-full h-[32px] px-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-500 dark:text-slate-500 cursor-not-allowed text-xs" />
                  </div>
                </div>
              </div>

              <div className="p-3 bg-gradient-to-r from-primary-500/10 to-violet-500/10 dark:from-primary-500/5 dark:to-violet-500/5 border border-primary-200/50 dark:border-primary-800/30 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Shield className="w-4 h-4 text-primary-500" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Change Password</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">Ensure your account is using a long, random password to stay secure.</p>
              ...
              <button type="submit" className="w-full h-[34px] bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-500 hover:to-primary-600 text-white px-4 rounded-lg font-semibold text-xs transition-all cursor-pointer shadow-xs shadow-primary-500/25 flex items-center justify-center">
                Save Profile
              </button>
            </form>
          )}
          ...
          {activeTab === 'system' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'system')} className="space-y-3 max-w-2xl text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <SettingsIcon className="w-4 h-4 text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Global Branding & Identity</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">Configure brand identity, regional formatting, and base security.</p>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div className="sm:col-span-2 border border-slate-100 dark:border-slate-800 rounded-lg p-2.5 bg-white dark:bg-slate-900/50 flex flex-col sm:flex-row items-center gap-3">
                    <div className="w-12 h-12 shrink-0 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center overflow-hidden border border-slate-200 dark:border-slate-700">
                      <img 
                    ...
                    <div className="flex-1">
                      <label className="block text-xs font-bold text-slate-800 dark:text-white mb-0.5">App Logo</label>
                      <p className="text-[10px] text-slate-400 mb-1.5">Upload a PNG, JPG, or SVG. Best size is 256x256px.</p>
                      <input type="file" accept="image/png, image/jpeg, image/svg+xml, image/webp" onChange={handleLogoUpload} className="block w-full text-[11px] text-slate-500 file:mr-3 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-[10px] file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 dark:file:bg-primary-500/10 dark:file:text-primary-400 dark:hover:file:bg-primary-500/20 cursor-pointer" />
                    </div>
                  </div>
                ...
                <div className="mt-3 flex items-center gap-2 p-2 bg-amber-50/50 dark:bg-amber-500/10 border border-amber-200/50 dark:border-amber-500/20 rounded-lg">
                  <input type="checkbox" id="maintenance" checked={systemForm.maintenance_mode} onChange={e => setSystemForm({...systemForm, maintenance_mode: e.target.checked})} className="w-3.5 h-3.5 text-primary-600 rounded border-slate-300 cursor-pointer" />
                  <label htmlFor="maintenance" className="font-semibold text-xs text-slate-700 dark:text-slate-300 cursor-pointer">
          ...
          {activeTab === 'financial' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'financial')} className="space-y-3 max-w-xl text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <DollarSign className="w-4 h-4 text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Financial Defaults</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">Configure baseline rates for commissions and TDS.</p>
          ...
          {activeTab === 'workflow' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'workflow')} className="space-y-3 max-w-xl text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Workflow className="w-4 h-4 text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Lead Workflow Rules</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">Set automation rules for leads and follow-ups.</p>
          ...
          {activeTab === 'smtp' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'smtp')} className="space-y-3 max-w-xl text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Mail className="w-4 h-4 text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">SMTP Server Configuration</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">Credentials for sending automated emails and receipts.</p>
          ...
          {activeTab === 'slideshow' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'slideshow')} className="space-y-3 max-w-2xl text-xs">
              <div className="p-3 bg-gradient-to-r from-primary-500/10 to-violet-500/10 dark:from-primary-500/5 dark:to-violet-500/5 border border-primary-200/50 dark:border-primary-800/30 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Sparkles className="w-4 h-4 text-primary-500" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Website Hero Slideshow Customization</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <div className="space-y-2.5">
                {[1, 2, 3, 4].map(num => {
                  ...
                  return (
                    <div key={num} className="p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl space-y-2.5">
                      <div className="flex items-center justify-between">
                        <h3 className="font-extrabold text-xs text-primary-650 dark:text-primary-400 uppercase tracking-wider">Slide {num} Settings</h3>
                        <span className="px-1.5 py-0.5 text-[9px] font-extrabold uppercase rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400">Landing Page</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <textarea rows={2} value={slideshowForm[descKey]} onChange={e => setSlideshowForm({...slideshowForm, [descKey]: e.target.value})} className={clsx(inputClass, 'min-h-[56px] resize-none')} />
                        </div>
                      </div>
                    </div>
                  );
          ...
          {activeTab === 'socials' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'socials')} className="space-y-3 max-w-xl text-xs">
              <div className="p-3 bg-gradient-to-r from-primary-500/10 to-violet-500/10 dark:from-primary-500/5 dark:to-violet-500/5 border border-primary-200/50 dark:border-primary-800/30 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Share2 className="w-4 h-4 text-primary-500" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Social Media & Contact Hub Links</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">
          ...
          {activeTab === 'security' && isAdmin && (
            <form onSubmit={e => handleSettingsSave(e, 'security')} className="space-y-3 max-w-2xl text-xs">
              <div className="p-3 bg-gradient-to-r from-rose-500/10 to-red-500/10 dark:from-rose-500/5 dark:to-red-500/5 border border-rose-200/50 dark:border-rose-500/20 rounded-xl">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Shield className="w-4 h-4 text-primary-500" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-white">Endpoint Rate Limiting & Auth Backoff</h2>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <div className="space-y-2.5">
                <div className="p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl space-y-2.5">
                  <h3 className="font-extrabold text-xs text-primary-650 dark:text-primary-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <div className="p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl space-y-2.5">
                  <h3 className="font-extrabold text-xs text-rose-650 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <button type="submit" className="w-full h-[34px] bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white px-4 rounded-lg font-semibold text-xs transition-all cursor-pointer shadow-xs shadow-rose-500/25 flex items-center justify-center">
