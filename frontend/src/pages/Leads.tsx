...
function LeadTableRow({
  ...
  return (
    <tr className={clsx(
      "group border-b border-slate-200 dark:border-slate-800/60 hover:bg-primary-100/50 dark:hover:bg-primary-500/10 transition-colors",
...
function KpiBadge({ statusKey, count, active, onClick }: { statusKey: string; count: number; active: boolean; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="block text-left w-full outline-none focus:ring-2 focus:ring-primary-500 rounded-xl transition-transform hover:scale-105 active:scale-95 cursor-pointer">
      <Card 
        hoverable 
        className={clsx(
          "p-3 flex items-center gap-3 transition-colors h-full w-full",
          active ? clsx(cfg.bg, cfg.border, 'ring-2 ring-primary-500 shadow-md') : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'
        )}
      >
        <div className={clsx('w-8 h-8 rounded-lg flex items-center justify-center shrink-0', active ? cfg.dot : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400', active && 'text-white shadow-inner')}>
          <Icon className="w-4 h-4" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest truncate">{cfg.label}</div>
          <div className={clsx('text-lg font-black mt-0.5 tabular-nums leading-none', active ? cfg.text : 'text-slate-800 dark:text-slate-100')}>{count}</div>
        </div>
      </Card>
    </button>
  );
  ...
  return (
    <button onClick={onClick}
      className={clsx(
        ...
        active
          ? 'bg-primary-600 text-white shadow-md shadow-primary-500/25 scale-100'
...
export default function Leads() {
  ...
  return (
    <div className="space-y-4 select-none">
      ...
      <Card className="overflow-hidden sticky top-14 z-30 mb-4 p-0">
        ...
        <div className="flex items-center overflow-x-auto border-b border-slate-100 dark:border-slate-800 px-3 py-1.5 bg-slate-50/50 dark:bg-slate-900/20">
          ...
          <button onClick={() => setShowFilters(!showFilters)}
            ...
            Filters {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-primary-600" />}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
                <div>
                  ...
                  <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                    className="w-full text-sm bg-slate-50 dark:bg-[#111622] text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-3 focus:border-primary-400 dark:focus:border-primary-500 focus:ring-2 focus:ring-primary-100 dark:focus:ring-primary-500/20 outline-none transition-colors">
      ...
      <Card className="overflow-hidden p-0 shadow-sm animate-in slide-in-from-bottom-2">
      ...
      {selectedLeadIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900/95 dark:bg-slate-950/95 text-white py-3 px-5 rounded-2xl shadow-2xl flex items-center gap-4 z-50 border border-slate-800/80 backdrop-blur-md text-xs font-bold">
          <span className="bg-primary-600 px-2.5 py-1 rounded-lg">
