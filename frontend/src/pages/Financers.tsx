        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
              {financer.contact_person && <span>{financer.contact_person}</span>}
              {financer.mobile && <span>• {financer.mobile}</span>}
            </div>
          </div>
        </div>
      </td>
...
export default function Financers() {
  ...
  const [formData, setFormData] = useState({
    name: '', contact_person: '', mobile: '', notes: '', is_active: 1
  });
  ...
  const handleOpenModal = (fin?: Financer) => {
    if (fin) {
      ...
      setFormData({
        name: fin.name || '',
        contact_person: fin.contact_person || '',
        mobile: fin.mobile || '',
        notes: fin.notes || '',
      ...
      setEditingFin(null);
      setFormData({ name: '', contact_person: '', mobile: '', notes: '', is_active: 1 });
    }
    setIsModalOpen(true);
  };
